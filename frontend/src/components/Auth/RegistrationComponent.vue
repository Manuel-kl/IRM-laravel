<template>
    ...
    <div v-if="!loading" class="flex flex-col bg-gray-100 items-center py-5">
        ...
        <form @submit.prevent="validateAndRegister" class="p-5 rounded-sm bg-gray-200 flex flex-col gap-y-5">
        ...
        <ErrorComponent :errors="store.state.authModule.errors" :error="registrationError" />
    </div>
</template>
<script setup>
